function [mw_gesamt, mw_beine, mw_arme, mw_kopf, mw_rumpf] = calc_mean_activation(reize)
%CALC_MEAN_ACTIVATION calculate mean activation value (per person and emotion)
    ...
    for condit=1:7
        for i=1:zeilen
            for j=1:spalten
                mw_gesamt(condit) = mw_gesamt(condit) + reize(i,j,condit); %mw enthält jetzt erstmal _Summen_werte, noch nicht _Mittel_werte!
