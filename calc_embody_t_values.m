function calc_embody_t_values(data_path)
if nargin < 1
    data_path = 'data';
end
...
for s=1:length(subjects) % loop over the subjects
    ...
    NC=length(data); % number of pictures/ of sub-folders in the folder corresponding to the current subject
    ...
    for s=1:length(subjects2); % loop over the subjects
    s
    % skip dot and dotdot folders
    if(strcmp(subjects2(s).name(1),'.')) 
        continue; 
    end 

    %% Data loading
    % let's load the subject's answers into a variable a
    data2=load_subj([basepath2 '/' subjects2(s).name],2);
    NC=length(data2); % number of pictures/ of sub-folders in the folder corresponding to the current subject
    ...
    % initialize the container for the data
    for n=1:NC;
        resmat2(:,:,n) = zeros(522,171); %zero matrix of the size of the base2 picture
        condition2(:,:,n) = zeros(522,171); %zero matrix of the size of the base2 picture
    end
    
    for n=1:NC; % loop over the pictures = conditions/ .csv-files
        T=length(data(n).paint(:,2)); % number of mouse locations
        over2=zeros(size(base,1),size(base,2)); % empty matrix to reconstruct painting
        for t=1:T
            y=ceil(data(n).paint(t,3)+1);
            x=ceil(data(n).paint(t,2)+1);
            if(x<=0) x=1; end
...
%     reize(:,:,1) = resmat(:,:,9) + resmat(:,:,17) + resmat(:,:,19) + resmat(:,:,21); % neutral
%     reize(:,:,2) = resmat(:,:,1) + resmat(:,:,5) + resmat(:,:,20) + resmat(:,:,22); %anger
%     reize(:,:,3)= resmat(:,:,2) + resmat(:,:,4) + resmat(:,:,11) + resmat(:,:,23); %disgust
%     reize(:,:,4)= resmat(:,:,3) + resmat(:,:,6) + resmat(:,:,12) + resmat(:,:,16); % happy
%     reize(:,:,5)= resmat(:,:,7) + resmat(:,:,14) + resmat(:,:,18) + resmat(:,:,24); %sadness
%     reize(:,:,6)= resmat(:,:,8) + resmat(:,:,10) + resmat(:,:,13) + resmat(:,:,15); %fear
    ...
    %% store result 
    save(['preprocessed_kontr_alexa/' subjects(s).name '_preprocessed.mat'],'resmat')
    save(['preprocessed_kontr_alexa/' subjects(s).name '_reize.mat'],'reize')
...
% As an example if we had a threshold, uncomment the below
if(0)
   th=0.2*Mtot; % just an example threshold, since this is not a statistical map 
   non_sig=round(th/M*NumCol); % proportion of non significant colors
   hotmap=hot(NumCol-non_sig);
   coldmap=flipud([hotmap(:,3) hotmap(:,2) hotmap(:,1) ]);
   hotcoldmap=[
